    ...
    val ddl =
      s"""
        ...
        if (timeExtractorFormatterPattern.nonEmpty) s" '" +
          s"${PARTITION_TIME_EXTRACTOR_FORMATTER_PATTEN.key()}' = '$timeExtractorFormatterPattern',"
        else ""
      }
         | 
...
class FiniteTestSource(elements: Iterable[Row], fun: Row => Long) extends SourceFunction[Row]
  ...
  override def run(ctx: SourceFunction.SourceContext[Row]): Unit = {
    ...
    lock.synchronized {
      for (t <- elements) {
        ctx.collect(t)
        ctx.emitWatermark(new Watermark(fun(t)))
      }
    }
