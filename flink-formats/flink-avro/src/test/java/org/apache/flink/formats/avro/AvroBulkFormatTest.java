...
public class AvroBulkFormatTest {
    ...
    public void testRestoreReader() throws IOException {
        ...
        Assert.assertEquals(offset1, offset2);
    }
    ...
    private void assertSplit(
        ...
        for (SplitInfo splitInfo : splitInfos) {
            ...
            for (int j = 1; j < offsets.size(); j++) {
                Assert.assertTrue(offsets.get(j - 1) < offsets.get(j));
            }
            reader.close();
        }
    }
    ...
    private long assertBatch(
        ...
        for (RecordAndPosition<RowData> recordAndPos = iterator.next();
            ...
            Assert.assertEquals(
                    TEST_DATA.get(batchInfo.start + skipCount), recordAndPos.getRecord());
            Assert.assertEquals(ret, recordAndPos.getOffset());
            skipCount++;
            Assert.assertEquals(skipCount, recordAndPos.getRecordSkipCount());
        }
        Assert.assertEquals(batchInfo.end - batchInfo.start, skipCount);
