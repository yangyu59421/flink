...
import static org.apache.flink.changelog.fs.ChangelogStorageMetricGroup.CHANGELOG_STORAGE_QUEUE_SIZE;
...
public class ChangelogStorageMetricsTest {
    ...
    public void testUploadsCounter() throws Exception {
        ...
        try (FsStateChangelogStorage storage =
            ...
            assertTrue(metrics.getUploadLatencies().getStatistics().getMin() > 0);
        }
    }
    ...
    @Test
    public void testUploadFailures() throws Exception {
        ...
        try (FsStateChangelogStorage storage =
            ...
            assertEquals(numUploads, metrics.getFailuresCounter().getCount());
        }
    }

    @Test
    public void testLogsPerUpload() throws Exception {
        ...
        try {
            ...
            assertEquals(numWriters, metrics.getLogsPerUpload().getStatistics().getMin());
            assertEquals(numWriters, metrics.getLogsPerUpload().getStatistics().getMax());
        } finally {
            storage.close();
        }
    }
                ...
                new ChangelogStorageMetricGroup(
                        new TaskManagerJobMetricGroup(
                                TestingMetricRegistry.builder()
                                        .setRegisterConsumer(
                                                (metric, name, unused) -> {
                                                    if (name.equals(CHANGELOG_STORAGE_QUEUE_SIZE)) {
