...
public class JdbcRowDataLookupFunctionTest extends JdbcLookupTestBase {
    ...
    @Test
    public void testEvalWithEmptyQueryResultPositive() throws Exception {
        ...
        expectedOutput.add(
                GenericRowData.of(
                        ...
                        StringData.fromString("49-c1"),
                        StringData.fromString("49-c2")));
        assertEquals(cache.getIfPresent(keyRow), expectedOutput);
    }

    @Test
    public void testEvalWithEmptyQueryResultNegative() throws Exception {

        JdbcLookupOptions lookupOptions =
                JdbcLookupOptions.builder()
                        .setExcludeEmptyQueryResult(false)
                        .setCacheExpireMs(60000)
                        .setCacheMaxSize(10)
                        .build();
        ...
        // put db entry for keyRow
        // final cache output should also be empty as we are including empty query result till TTL
        // expires
        insert(
                "INSERT INTO "
                        + LOOKUP_TABLE
